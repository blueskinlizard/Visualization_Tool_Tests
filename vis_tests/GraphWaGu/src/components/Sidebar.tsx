...
const dataset_list = ['sf_ba6000', 'fe_4elt2.mtx', 'pkustk02.mtx', 'pkustk01.mtx', 'finance256.mtx', 'benchmark_dataset'];

const datasets: Record<string, () => Promise<any>> = {
...
class Sidebar extends React.Component<SidebarProps, SidebarState> {
  ...
  constructor(props: SidebarProps | Readonly<SidebarProps>) {
    ...
    this.loadBenchmarkDataset = this.loadBenchmarkDataset.bind(this);
  }
  ...
  async loadSelectedDataset() {
    ...
    if (dataset === 'benchmark_dataset') {
      await this.loadBenchmarkDataset();
    } else {
      const graph = (await datasets[dataset as keyof typeof datasets]()) as unknown as Graph;
      this.loadGraph(graph);
    }
    
    setTimeout(() => {
      this.props.setNodeEdgeData(
        this.state.nodeData, 
        this.state.edgeData, 
        this.state.sourceEdges, 
  ...
  loadGraph(graph: Graph) {
    ...
    for (let i = 0; i < graph.nodes.length; i++) {
      if (graph.nodes[i].x) {
        ...
        nodeData.push(
          0.0, 
          Math.random() * Math.max(1, (graph.nodes.length / 100000)),
          Math.random() * Math.max(1, (graph.nodes.length / 100000)),
          1.0
        );
      }
  ...
  async loadBenchmarkDataset() {
    ...
    await new Promise((resolve) => {
      Papa.parse(basePath + 'dataset_2/dataset_edges.csv', {
        download: true,
        header: true,
        dynamicTyping: true,
        step: (row) => {
          if (edgesList.length >= this.state.edgeLimit) return;
          
          const row_data = row.data as any;
          const sourceId = parseInt(row_data.source_id);
          const targetId = parseInt(row_data.target_id);
          ...
          neededNodeIds.add(String(sourceId));
          neededNodeIds.add(String(targetId));
        },
        complete: () => {
          resolve(null);
        }
    ...
    this.timingsRef.edge_parsing = Date.now() - startEdgeParse;
    console.log(`Edge Parsing: ${this.timingsRef.edge_parsing}ms`);
    ...
    await new Promise((resolve) => {
      Papa.parse(basePath + 'dataset_2/dataset_nodes.csv', {
        download: true,
        header: true,
        dynamicTyping: true,
        step: (row, parser) => {
    ...
    this.timingsRef.node_parsing = Date.now() - startNodeParse;
    console.log(`Node Parsing: ${this.timingsRef.node_parsing}ms`);
    ...
    this.timingsRef.data_transformation = Date.now() - startTransform;
    console.log(`Data Transformation: ${this.timingsRef.data_transformation}ms`);

    // Benchmark 4: Graph Structure Preparation
    ...
    this.timingsRef.graph_preparation = Date.now() - startGraphPrep;
    console.log(`Graph Preparation: ${this.timingsRef.graph_preparation}ms`);

    this.loadGraph(graph);
    
    this.timingsRef.total_time = Date.now() - this.startTimeRef;
    console.log(`Total Time: ${this.timingsRef.total_time}ms`);
    
    this.props.setBenchmarks({
      ...this.timingsRef,
  ...
  async chooseDataset(dataset: keyof typeof datasets | 'benchmark_dataset') {
          ...
          <Form style={{ color: 'white' }} onSubmit={this.handleSubmit}>
            <Form.Group controlId="formFile" className="mt-3 mb-3">
              ...
              <Select 
                ...
                onChange={(e) => this.chooseDataset(e!.value as any)} 
                options={dataset_list.map((cm) => { 
                  return { "label": cm, "value": cm } 
                })}
              />
              
              {this.state.selectedDataset === 'benchmark_dataset' && (
                <>
                  <Form.Label className="mt-3">Edge Limit for Benchmark Dataset:</Form.Label>
                  <Form.Control
                    type="number"
                    value={this.state.edgeLimit}
                    min={1000}
                    max={50000}
                    step={1000}
                    onChange={(e) => this.setState({ edgeLimit: parseInt(e.target.value) })}
                  />
                </>
              )}
              
              <Form.Label className="mt-3">Choose your own JSON file...</Form.Label>
              <Form.Control 
                className="form-control" 
                type="file" 
                multiple 
                onChange={(e) => { this.readJson(e as React.ChangeEvent<HTMLInputElement>) }} 
              />
              <Button className="mt-2" type="submit" variant="secondary" value="Submit">
                Submit
              </Button>
            </Form.Group>
            ...
            <Collapsible trigger="Layers" open={true}>
              <Form.Check 
                defaultChecked={true} 
                onClick={() => this.props.toggleNodeLayer()} 
                type="checkbox" 
                label="Node Layer" 
              />
              <Form.Check 
                defaultChecked={true} 
                onClick={() => this.props.toggleEdgeLayer()} 
                type="checkbox" 
                label="Edge Layer" 
              />
            </Collapsible>
